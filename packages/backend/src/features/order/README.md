...
The order statuses are `PENDING`, `COMPLETE`, and `CANCELLED`. Unique partial indexes protect active customer/listing and listing/slot ownership. The feature applies immutable reservation facts with an upsert by `orderId`. It does not change an existing order's status, reservation fields, or timestamps. It does not implement order transitions or reconciliation.
