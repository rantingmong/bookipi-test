...
describe('validateRequest', () => {
  it('returns structured 400 issues and stops invalid requests', async () => {
    ...
    app.post('/example', validateRequest('body', z.object({ count: z.number().int().positive() })), (_request, response) => {
    ...
    try {
      ...
      expect(await response.json()).toMatchObject({ error: 'Invalid request', issues: [{ path: ['count'] }] })
    } finally {
      server.close()
    }
  })
