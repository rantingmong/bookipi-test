...
describe('requireSession', () => {
  async function requestWith(resolveSession: Parameters<typeof requireSession>[0]) {
    const app = express()
    let incomingRequest: express.Request | undefined
    const protectedHandler = vi.fn((request: express.Request, response: express.Response) => {
    ...
    try {
      ...
      return { response, body: await response.json(), protectedHandler, errorHandler, incomingRequest }
    } finally {
      server.close()
    }
  }
  ...
  it('returns the authentication error and stops a request with no identity', async () => {
    ...
    const { response, body, protectedHandler, incomingRequest } = await requestWith(resolveSession)
  ...
  it('forwards a session resolution error and stops the protected route', async () => {
    ...
    const { response, body, protectedHandler, incomingRequest, errorHandler } = await requestWith(resolveSession)
