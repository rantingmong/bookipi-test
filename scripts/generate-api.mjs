...
async function resolveList(ownerPath, key) {
  ...
  if (!Array.isArray(list))
    throw new Error(`${ownerPath} must define ${key} as a list`)
...
async function generateGroup(groupConfigPath) {
    ...
    for (const [route, methods] of Object.entries(endpoint.paths ?? {})) {
      for (const [method, operation] of Object.entries(methods)) {
        ...
        if (seenOperations.has(key))
          throw new Error(`Duplicate API operation: ${key}`)
  ...
  const generatorConfig = resolve(
    generatedRoot,
    `${groupName}.server.config.json`,
  )
  ...
  await execFileAsync('pnpm', [
    ...
    'openapi-server',
    '--config',
    relative(root, generatorConfig),
  ])

  const bindings = operations.map(({ operationId, endpointPath }) => {
    if (!operationId)
      throw new Error(`${endpointPath} operation is missing operationId`)
    const handlerPath = relative(
      dirname(groupConfigPath),
      resolve(dirname(endpointPath), 'handler.ts'),
    ).replaceAll(sep, '/')
  ...
  const imports = bindings
    .map(
      ({ operationId, importPath }, index) =>
        `import { ${operationId} as handler${index} } from '${importPath}'`,
    )
    .join('\n')
  const entries = bindings
    .map(({ operationId }, index) => `  ${operationId}: handler${index},`)
    .join('\n')
  await writeFile(
    resolve(generatedDirectory, 'handlers.ts'),
...
await mkdir(generatedRoot, { recursive: true })
const rootGroups = await resolveList(
  resolve(apiRoot, 'openapi.yml'),
  'x-groups',
)
...
await execFileAsync('pnpm', [
  ...
  rootSpecPath,
  '--output',
  resolve(generatedRoot, 'api.openapi.yaml'),
  '--ext',
  'yaml',
])
const storefrontGenerated = resolve(
  root,
  'packages/storefront/src/lib/api/generated',
)
