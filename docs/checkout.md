...
An authorizer Deny policy stops the request before the checkout Lambda runs. For a cross-origin deployment, an API Gateway `GatewayResponse` for authorizer denials must return the exact allowed `Origin` and `Access-Control-Allow-Credentials: true`. Otherwise, the browser can report an opaque network error. No deployment configuration or proof exists in this repository.
...
Every checkout Lambda response sets `Cache-Control: no-store`. When the request `Origin` exactly matches configured `STOREFRONT_ORIGIN`, accepted responses and relevant errors include that origin in `Access-Control-Allow-Origin` and set `Access-Control-Allow-Credentials: true`. The handler does not echo another origin. It does not handle `OPTIONS`.

The storefront shows accepted, retryable, unauthenticated, sold-out, and rejected states. An explicit `401` or `UNAUTHENTICATED` response shows sign-in guidance. After an opaque network failure or a readable `403` without a checkout error code, it revalidates the Better Auth session. It shows sign-in guidance when the session is absent. It keeps the request retryable when the session remains active or the revalidation request fails. It preserves explicit checkout errors without session revalidation. A retry reuses the same idempotency key. A successful response opens the relative payment redirect. The browser cannot supply `customerId`.
