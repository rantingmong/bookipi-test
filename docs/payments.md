...
After the outcome route returns successfully, the page opens `/order-status?orderId=...`. That page reads the order again through the authenticated `GET /api/orders/{orderId}` route. It shows the current status only to the order owner. A failed outcome request does not redirect.
...
- Added one order status page after successful and cancelled mock outcomes.
